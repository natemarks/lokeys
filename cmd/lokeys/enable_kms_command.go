...
func (c *enableKMSCommand) Execute(_ context.Context, f *flag.FlagSet, _ ...interface{}) subcommands.ExitStatus {
	return runWithExitStatus(runEnableKMS(f.Args(), c.alias, c.region, c.apply))
}

func runEnableKMS(args []string, alias string, region string, apply bool) error {
	if err := requireNoArgs(args, "enable-kms"); err != nil {
		return err
	}
	message, err := lokeys.RunEnableKMS(lokeys.EnableKMSOptions{Alias: alias, Region: region, Apply: apply})
	if err != nil {
		return err
	}
	fmt.Println(message)
